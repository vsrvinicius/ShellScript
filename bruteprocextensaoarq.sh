...
echo ""
echo "  *****************************************************"
echo " *** Programa para Web Recon - Extensão de Arquivos ***"
echo " *****************************************************"
echo ""
...
read site1
echo "Qual extensão procurar?(php,txt,pdf, xml...)"
read ext
	...
	read usragent
		...
		then
			...
			do
				...
				then
					...
					do
						rarquivo=$(curl -s -H "User-Agent: $x" -e robots.txt -o /dev/null -w "%{http_code}" $site/$arquivo.$ext)
						if [ $rarquivo == "200" ]
						then
							echo "	-> Arquivo encontrado: $site/$arquivo.$ext"
							echo "	-> Arquivo encontrado: $site/$arquivo.$ext" >> $site_$ext.txt
							let totalarq+=1;
						fi
					done
					...
					do
						...
						then
							...
							for arquivo in $(cat $word);
							do
								rarquivo=$(curl -s -H "User-Agent: $x" -e robots.txt -o /dev/null -w "%{http_code}" $site/$palavra/$arquivo.$ext)
								if [ $rarquivo == "200" ]
								then
									echo "	-> Arquivo encontrado: $site/$palavra/$arquivo.$ext"
									echo "	-> Arquivo encontrado: $site/$arquivo.$ext" >> $site_$ext.txt
									let totalarq+=1;
								fi
							done
		...
		resposta1=$(curl -s -e robots.txt -o /dev/null -w "%{http_code}" $ssite)
			...
			then
				...
				for arquivo in $(cat $word);
					do
						rarquivo=$(curl -s -e robots.txt -o /dev/null -w "%{http_code}" $site/$arquivo.$ext)
						if [ $rarquivo == "200" ]
						then
							echo "	-> Arquivo encontrado: $site/$arquivo.$ext"
							let totalarq+=1;
					...
					do
						...
						then
							...
							for arquivo in $(cat $word);
							do
								rarquivo=$(curl -s -e robots.txt -o /dev/null -w "%{http_code}" $site/$palavra/$arquivo.$ext)
								if [ $rarquivo == "200" ]
								then
									echo "	-> Arquivo encontrado: $site/$palavra/$arquivo.$ext"
									let totalarq+=1;
